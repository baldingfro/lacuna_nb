...
local SCRIPTNAME = "menu_lnb_ship_comparison.lua"
local SCRIPTPAGE = "698008001"

local menu = {
	name = "LNBTimelineMenu",
	dateFormat = "nt",
...
local config = {
	leftBar = {
		[1] = { name = ReadText(SCRIPTPAGE, 2400),	icon = "tlt_lacuna_nb",		mode = "encyclopedia",	condition = function () return C.IsStoryFeatureUnlocked("x4ep1_encyclopedia") end },
...
function menu.buttonToggleEncyclopediaMode(mode)
	...
	elseif mode == "encyclopedia" then
		Helper.closeMenuAndOpenNewMenu(menu, "LacunaNBMenu", { 0, 0 }, true)
		menu.cleanup()
	...
	for _, entry in ipairs(menu.timeline) do
		...
		if isknown then
			if C.IsKnownItemRead("timeline", entry.id) then
				row[2]:setBackgroundColSpan(3):createText(menu.convertDate(entry.date), { halign = "right", font = Helper.standardFontBold, helpOverlayID = "timeline_entry_" .. entry.id, helpOverlayText = " ", helpOverlayHighlightOnly = true, helpOverlayUseBackgroundSpan = true })
